...
export async function projectAudience(db: DbQueryable, projectId: number): Promise<ProjectAudience> {
...
export async function assertMemberCanUseProject(db: DbQueryable, memberId: number, projectId: number): Promise<Project> {
  const [member, data] = await Promise.all([findMemberById(db, memberId), listBootstrap(db)]);
  const project = data.projects.find((item) => item.id === projectId);
  if (!member || !project || !member.projectIds.includes(projectId)) throw new Error("Project access is not available");
  return project;
}

export async function resolveMemberProject(db: DbQueryable, memberId: number, requestedProjectId?: number): Promise<MemberProjectScope> {
  ...
  if (!selectedProject) throw new Error("Project access is not available");
  return { ...(await projectAudience(db, selectedProject.id)), selectableProjects };
}

export async function scopedEntries(
  db: DbQueryable,
  projectId: number,
  ...
  const audienceIds = (await projectAudience(db, projectId)).members.map((member) => member.id);
  const memberIds = requestedMemberIds.length
    ? audienceIds.filter((id) => requestedMemberIds.includes(id))
    : audienceIds;
  if (!memberIds.length) return [];
  return (await listEntries(db, fromUtc, toUtc, memberIds)).map((entry) => {
