...
export async function GET(request: Request) {
  ...
  try {
    ...
    const members = (await projectAudience(db, projectId)).members;
    const entries = await scopedEntries(db, projectId, memberId, from.toISO()!, to.toISO()!);
    return NextResponse.json(buildDashboard({ now: now.toISO()!, zone, members, entries }));
  } catch (error) {
    return NextResponse.json({ error: publicDatabaseError(error, errorMessage(error)) }, { status: isProjectAccessError(error) ? 403 : 400 });
  }
}
