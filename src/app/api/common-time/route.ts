...
export async function POST(request: Request) {
  ...
  try {
    ...
    const db = getDb();
    assertMemberCanUseProject(db, memberId, input.projectId);
    assertRequestedMembersInProject(db, input.projectId, input.memberIds);
    const entries = scopedEntries(db, input.projectId, memberId, input.fromUtc, input.toUtc, input.memberIds);
